...
export async function POST(request: Request) {
  try {
    const profile = scoringProfileSchema.parse(await request.json());
    const { data, etag } = await readScoringProfiles();
    if (data.profiles.some((p) => p.profile_id === profile.profile_id)) {
      return NextResponse.json(
        { error: `Scoring profile '${profile.profile_id}' already exists.` },
        { status: 409 }
      );
    }
    await writeScoringProfiles({ profiles: [...data.profiles, profile] }, etag);
